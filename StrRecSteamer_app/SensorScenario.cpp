...
bool SensorScenario::getWriterState()
{
	
	if (m_pLONGTHROWProcessor != nullptr && m_pLONGTHROWProcessor->getWriterState())
		return true;
	return false;
}
...
void SensorScenario::InitializeCameraReaders(MixedReality* mixedReality)
{
	...
	if (m_pLFCameraSensor)
	{
		auto cameraReader = std::make_shared<RMCameraReader>(m_pLFCameraSensor, camConsentGiven, &camAccessCheck, guid, m_mixedReality);
		m_cameraReaders.push_back(cameraReader);
	}

	if (m_pRFCameraSensor)
	{
		auto cameraReader = std::make_shared<RMCameraReader>(m_pRFCameraSensor, camConsentGiven, &camAccessCheck, guid, m_mixedReality);
		m_cameraReaders.push_back(cameraReader);
	}

	if (m_pLLCameraSensor)
	{
		auto cameraReader = std::make_shared<RMCameraReader>(m_pLLCameraSensor, camConsentGiven, &camAccessCheck, guid, m_mixedReality);
		m_cameraReaders.push_back(cameraReader);
	}

	if (m_pRRCameraSensor)
	{
		auto cameraReader = std::make_shared<RMCameraReader>(m_pRRCameraSensor, camConsentGiven, &camAccessCheck, guid, m_mixedReality);
		m_cameraReaders.push_back(cameraReader);
	}

	if (m_pLTSensor)
	{
		auto processor = std::make_shared<RMCameraReader>(m_pLTSensor, camConsentGiven, &camAccessCheck, guid,m_mixedReality);
	...
	if (m_pAHATSensor)
	{
		auto processor = std::make_shared<RMCameraReader>(m_pAHATSensor, camConsentGiven, &camAccessCheck, guid, m_mixedReality);
		m_pAHATProcessor = processor;
	}	
}

void SensorScenario::StartRecording(const winrt::Windows::Perception::Spatial::SpatialCoordinateSystem& worldCoordSystem)
{
	for (int i = 0; i < m_cameraReaders.size(); ++i)
	{
		m_cameraReaders[i]->SetWorldCoordSystem(worldCoordSystem);		
	}
	if (m_pLONGTHROWProcessor)
	{
		m_pLONGTHROWProcessor->SetWorldCoordSystem(worldCoordSystem);
	}
	if (m_pAHATProcessor)
	{
		m_pAHATProcessor->SetWorldCoordSystem(worldCoordSystem);
	}
}
